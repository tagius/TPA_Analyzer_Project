"""Core TPA analysis implementation."""
...
@dataclass
class TPAConfig:
    """Configuration values controlling TPA cycle detection and calculations."""
...
def _detect_header_line(lines: list[str]) -> tuple[int, str]:
    """Locate the most likely header row and delimiter in a Zwick export."""
    delimiters = [",", ";", "\t"]
...
def parse_zwick_data(filepath: str | Path) -> pd.DataFrame:
    """Parse a Zwick CSV or TRA file into normalized numeric columns."""
    path = Path(filepath)
    ...
    if missing:
        raise DataParseError(f"Missing required columns {missing} in file: {path.name}")
    ...
    df = df[required]
    if df.empty:
        raise DataParseError(f"No valid numeric data found in file: {path.name}")
    return df


def _find_crossing_start(force: np.ndarray, peak_idx: int, threshold: float) -> int:
    """Walk backward from a peak until the threshold is crossed."""
    idx = peak_idx
...
def _find_crossing_end(force: np.ndarray, peak_idx: int, threshold: float) -> int:
    """Walk forward from a peak until the threshold is crossed."""
    idx = peak_idx
    n_points = len(force)
    while idx < n_points - 1 and force[idx] > threshold:
        idx += 1
    return idx


def _integrate_segment(time_vals: np.ndarray, signal_vals: np.ndarray, positive: bool) -> float:
    """Integrate positive or negative segments of a trace."""
    if len(time_vals) < 2:
        return 0.0
...
def _detect_two_peaks(force: np.ndarray, config: TPAConfig) -> np.ndarray:
    """Detect the first two compression peaks using progressive fallbacks."""
    attempts = [
...
def build_metrics_row(result: dict[str, Any], filename: str, group: str) -> dict[str, Any]:
    """Build a normalized metrics row from an analysis result payload."""
    return {
        "Filename": filename,
        "Group": group,
...
def calculate_tpa(
    ...
    """Calculate TPA metrics and return metrics plus trace-ready data for plotting."""
    if config is None:
        config = TPAConfig()
    if df.empty:
        raise AnalysisError("Input dataframe is empty.")
    ...
    if len(peaks) < 2:
        raise AnalysisError("Could not detect two distinct compression cycles.")
...
def generate_plots(df: pd.DataFrame, filename: str, output_dir: str = "output_plots") -> str:
    """Export a backward-compatible per-file raw trace figure."""
    import matplotlib
